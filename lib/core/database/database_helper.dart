...
class DatabaseHelper {
  ...
  Future<void> recordSale({required String invoiceNo, int? customerId, required String customerName, required List<Map<String,dynamic>> items, required double discount, required String paymentMethod, required double paid, required String notes}) async { final db=await database; await db.transaction((txn) async { final subtotal=items.fold<double>(0,(sum,item)=>sum+((item['qty'] as num)* (item['price'] as num))); final tax=(subtotal-discount)*.15; final total=subtotal-discount+tax; final due=total-paid; final invoiceId=await txn.insert('invoices',{'inv_no':invoiceNo,'number':invoiceNo,'date':DateTime.now().toIso8601String(),'customer_id':customerId,'customer_name':customerName,'total':total,'subtotal':subtotal,'discount':discount,'tax':tax,'paid':paid,'due':due,'payment_method':paymentMethod,'notes':notes}); for(final item in items){await txn.insert('invoice_items',{'invoice_id':invoiceId,'product_id':item['product_id'],'product_name':item['name'],'qty':item['qty'],'quantity':item['qty'],'price':item['price'],'discount':item['discount']??0,'total':(item['qty'] as num)*(item['price'] as num)}); if(item['product_id']!=null){await txn.rawUpdate('UPDATE products SET quantity=quantity-? WHERE id=?',[item['qty'],item['product_id']]);}} await txn.insert('payments',{'invoice_id':invoiceId,'amount':paid,'method':paymentMethod,'date':DateTime.now().toIso8601String()}); await txn.insert('journal',{'date':DateTime.now().toIso8601String(),'account_debit':customerName=='نقدي'?paymentMethod:'العملاء','account_credit':'المبيعات','amount':total,'description':'بيع فاتورة $invoiceNo','ref_id':invoiceId}); await txn.insert('journal',{'date':DateTime.now().toIso8601String(),'account_debit':'تكلفة البضاعة','account_credit':'المخزون','amount':items.fold<double>(0,(s,i)=>s+((i['qty'] as num)*(i['cost'] as num? ?? 0))),'description':'تكلفة فاتورة $invoiceNo','ref_id':invoiceId}); }); }
  Future<void> addExpense(String title,double amount,String category) async { final db=await database; await db.transaction((txn) async { final id=await txn.insert('expenses',{'title':title,'amount':amount,'date':DateTime.now().toIso8601String(),'category':category}); await txn.insert('journal',{'date':DateTime.now().toIso8601String(),'account_debit':category,'account_credit':'الصندوق','amount':amount,'description':title,'ref_id':id}); }); }
  Future<List<Map<String,dynamic>>> journalRows() => all('journal');
  Future<List<double>> dailySales() async { final db=await database; final now=DateTime.now(); final out=<double>[]; for(var i=6;i>=0;i--){final s=DateTime(now.year,now.month,now.day-i);final e=s.add(const Duration(days:1));final rows=await db.rawQuery('SELECT COALESCE(SUM(total),0) v FROM invoices WHERE date>=? AND date<?',[s.toIso8601String(),e.toIso8601String()]);out.add((rows.first['v'] as num).toDouble());} return out; }
