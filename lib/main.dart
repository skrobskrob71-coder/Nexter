...
class _NewInvoicePageState extends State<NewInvoicePage> {
  ...
  @override Widget build(BuildContext context) { final total = subtotal * 1.15; final remaining = total - (double.tryParse(paid.text) ?? 0); return Scaffold(appBar: AppBar(title: const Text('الكاشير • فاتورة جديدة')), body: ListView(padding: const EdgeInsets.all(16), children: [Row(children: [Expanded(child: TextField(controller: search, onSubmitted: (value) { final found = products.where((p) => p['name'].toString().contains(value) || p['barcode'].toString() == value).toList(); if (found.isNotEmpty) addProduct(found.first); }, decoration: const InputDecoration(labelText: 'بحث عن منتج أو باركود', prefixIcon: Icon(Icons.search)))), const SizedBox(width: 8), IconButton.filled(onPressed: scan, icon: const Icon(Icons.qr_code_scanner), tooltip: 'مسح باركود')]), const SizedBox(height: 8), OutlinedButton.icon(onPressed: pickProduct, icon: const Icon(Icons.add_shopping_cart), label: const Text('إضافة منتج من القائمة')), const SizedBox(height: 12), if (items.isEmpty) const EmptyState(icon: Icons.shopping_cart_outlined, text: 'لم تتم إضافة منتجات') else ...items.asMap().entries.map((entry) { final item = entry.value; return Card(child: ListTile(title: Text(item['name'].toString()), subtitle: Text('الكمية: ${item['qty']} • السعر: ${money.format(item['price'])}'), trailing: Row(mainAxisSize: MainAxisSize.min, children: [Text(money.format((item['qty'] as num) * (item['price'] as num))), IconButton(onPressed: () => setState(() => items.removeAt(entry.key)), icon: const Icon(Icons.delete_outline, color: Colors.red))])); }), const SizedBox(height: 12), Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(children: [DropdownButtonFormField<int?>(value: customerId, decoration: const InputDecoration(labelText: 'العميل'), items: [const DropdownMenuItem<int?>(value: null, child: Text('نقدي')), ...customers.map((c) => DropdownMenuItem<int?>(value: c['id'] as int, child: Text(c['name'].toString())))], onChanged: (value) => setState(() { customerId = value; customerName = value == null ? 'نقدي' : customers.firstWhere((c) => c['id'] == value)['name'].toString(); })), const SizedBox(height: 12), DropdownButtonFormField<String>(value: payment, decoration: const InputDecoration(labelText: 'طريقة الدفع'), items: const [DropdownMenuItem(value: 'كاش', child: Text('كاش')), DropdownMenuItem(value: 'شبكة', child: Text('شبكة')), DropdownMenuItem(value: 'أجل', child: Text('أجل'))], onChanged: (value) => setState(() => payment = value ?? 'كاش')), const SizedBox(height: 12), TextField(controller: paid, onChanged: (_) => setState(() {}), keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'المبلغ المدفوع')), _line('المجموع', subtotal), _line('الضريبة 15%', subtotal * .15), _line('الإجمالي', total, bold: true), _line('المتبقي', remaining, bold: true)]))), const SizedBox(height: 14), Row(children: [Expanded(child: OutlinedButton(onPressed: () => save(print: false), child: const Text('حفظ فقط'))), const SizedBox(width: 10), Expanded(child: FilledButton.icon(onPressed: () => save(print: true), icon: const Icon(Icons.print), label: const Text('حفظ وطباعة')))])])); }
