...
class NaxterApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'ناكستر Naxter', debugShowCheckedModeBanner: false, locale: const Locale('ar'),
      theme: ThemeData(useMaterial3: true, colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFF0D47A1)), fontFamily: 'Cairo', scaffoldBackgroundColor: const Color(0xFFF5F5F5)),
...
class _SplashPageState extends State<SplashPage> {
  ...
  @override Widget build(BuildContext context) => const Scaffold(backgroundColor: Color(0xFF0D47A1), body: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Text('N', style: TextStyle(color: Colors.white, fontSize: 100, fontWeight: FontWeight.bold)), Text('ناكستر Naxter', style: TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.bold))])));
}

class LoginPage extends StatelessWidget { const LoginPage({super.key});
  @override Widget build(BuildContext context) => Scaffold(body: Center(child: Card(margin: const EdgeInsets.all(24), child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [const Text('تسجيل الدخول', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), const SizedBox(height: 18), const TextField(decoration: InputDecoration(labelText: 'اسم المستخدم', border: OutlineInputBorder())), const SizedBox(height: 12), const TextField(obscureText: true, decoration: InputDecoration(labelText: 'كلمة المرور', border: OutlineInputBorder())), const SizedBox(height: 18), SizedBox(width: double.infinity, child: FilledButton(onPressed: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const HomePage())), child: const Text('تسجيل الدخول'))]))));
}
...
class _HomePageState extends State<HomePage> {
  ...
  @override Widget build(BuildContext context) { final pages = [const DashboardPage(), const InvoicesPage(), const CustomersPage(), const ProductsPage(), const ReportsPage()]; return Scaffold(appBar: AppBar(title: Text(titles[index])), drawer: Drawer(child: ListView(children: [const DrawerHeader(decoration: BoxDecoration(color: Color(0xFF0D47A1)), child: Center(child: Text('ناكستر Naxter', style: TextStyle(color: Colors.white, fontSize: 24)))), ListTile(leading: const Icon(Icons.settings), title: const Text('الإعدادات'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsPage()))) ])), body: Directionality(textDirection: TextDirection.rtl, child: pages[index]), bottomNavigationBar: NavigationBar(selectedIndex: index, onDestinationSelected: (v) => setState(() => index = v), destinations: const [NavigationDestination(icon: Icon(Icons.dashboard), label: 'الرئيسية'), NavigationDestination(icon: Icon(Icons.receipt), label: 'الفواتير'), NavigationDestination(icon: Icon(Icons.people), label: 'العملاء'), NavigationDestination(icon: Icon(Icons.inventory), label: 'المنتجات'), NavigationDestination(icon: Icon(Icons.bar_chart), label: 'التقارير')])); }
}

class DashboardPage extends StatelessWidget { const DashboardPage({super.key});
  @override Widget build(BuildContext context) => FutureBuilder<List<double>>(future: DatabaseHelper.instance.monthlySales(), builder: (context, snapshot) { final values = snapshot.data ?? List<double>.filled(6, 0); return ListView(padding: const EdgeInsets.all(16), children: [const Text('ملخص الحسابات', style: TextStyle(fontSize: 23, fontWeight: FontWeight.bold, color: Color(0xFF0D47A1))), const SizedBox(height: 16), Wrap(spacing: 8, runSpacing: 8, children: const [StatCard('المبيعات', Icons.trending_up), StatCard('المشتريات', Icons.shopping_cart), StatCard('العملاء', Icons.people), StatCard('صافي الربح', Icons.account_balance_wallet)]), const SizedBox(height: 20), Card(child: Padding(padding: const EdgeInsets.all(12), child: SizedBox(height: 220, child: LineChart(LineChartData(minY: 0, gridData: const FlGridData(show: true), titlesData: const FlTitlesData(rightTitles: AxisTitles(sideTitles: SideTitles(showTitles: false)), topTitles: AxisTitles(sideTitles: SideTitles(showTitles: false))), lineBarsData: [LineChartBarData(isCurved: true, color: const Color(0xFF0D47A1), barWidth: 3, spots: [for (var i = 0; i < values.length; i++) FlSpot(i.toDouble(), values[i])])])))), const Text('مبيعات آخر 6 شهور', textAlign: TextAlign.center)]; });
}
...
class ReportsPage extends StatelessWidget { const ReportsPage({super.key}); @override Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [for (final title in ['تقرير المبيعات', 'تقرير الأرباح والخسائر', 'كشف حساب عميل']) Card(child: ListTile(leading: const Icon(Icons.assessment), title: Text(title), onTap: () {}))]); }
class SettingsPage extends StatelessWidget { const SettingsPage({super.key}); @override Widget build(BuildContext context) => ListView(children: [const ListTile(leading: Icon(Icons.business), title: Text('بيانات الشركة')), ListTile(leading: const Icon(Icons.print), title: const Text('اختبار طباعة PDF'), onTap: () => PdfService.printInvoice(number: 'TEST-001', customer: 'عميل تجريبي', subtotal: 100, tax: 15, total: 115)), const ListTile(leading: Icon(Icons.backup), title: Text('النسخ الاحتياطي')), const ListTile(leading: Icon(Icons.info), title: Text('حول التطبيق'))]); }
